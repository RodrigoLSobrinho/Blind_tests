...
def plot_pred_vs_measured(predicted_data: pd.DataFrame, 
    ...
    for model_name in models_found:
        ...
        if prediction_column in predicted_data.columns:
            ...
            plt.scatter(y_true_clean, y_pred_clean, alpha=0.6, color='steelblue', s=50)
            
            # Add 1:1 line
            min_val = min(min(y_true_clean), min(y_pred_clean))
            max_val = max(max(y_true_clean), max(y_pred_clean))
            plt.plot([min_val, max_val], [min_val, max_val], color='darkred', linestyle='--', linewidth=2, alpha=0.8)
            
            plt.xlabel("COT Medido(%)", fontsize=12)
            plt.ylabel("COT Predito(%)", fontsize=12)
            plt.title(f"Predito vs Medido - {model_name}", fontsize=14)
            ...
            # Add metrics text
            plt.text(0.05, 0.95, f'RMSLE = {rmsle:.4f}\nMALE = {male:.4f}%', 
...
def plot_predictions(predicted_data: pd.DataFrame, 
    ...
    fig.suptitle('Comparação de Predição', fontsize=16)
            ...
            if 'COT' in predicted_data.columns:
                ...
                if mask_cot.any():
                    ax.scatter(predicted_data.loc[mask_cot, 'COT'], predicted_data.loc[mask_cot, 'DEPTH'],
                               label="COT Medido", color='steelblue', marker='o', s=20, alpha=0.6)
            ...
            if 'COT_anl' in predicted_data.columns:
                ...
                if mask_cot_anl.any():
                    ax.scatter(predicted_data.loc[mask_cot_anl, 'COT_anl'], predicted_data.loc[mask_cot_anl, 'DEPTH'],
                               label="COT Analisado", color='dimgray', marker='*', s=30, alpha=0.9)
            
            ax.plot(predicted_data[prediction_column], predicted_data['DEPTH'], 
                   color="darkred", label=f"COT Predito", alpha=0.8, linewidth=1)
            
            ax.set_xlim(0, 15)
            ax.set_xlabel("COT (%)", fontsize=13)
            ax.set_ylabel("Profundidade (m)" if i == 0 else "", fontsize=13)
...
def plot_residuals(predicted_data: pd.DataFrame, 
    ...
    for model_name in models_found:
        ...
        if prediction_column in predicted_data.columns:
            ...
            fig, ((ax2, ax1), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
            #fig.suptitle(f'Residual Analysis - {model_name}', fontsize=16, fontweight='bold')
            
            # 1. Residuals vs Predicted Values
            ax1.scatter(y_pred_clean, residuals, alpha=0.8, color='tan', s=30)
            ax1.axhline(y=0, color='darkred', linestyle='--', alpha=0.8, linewidth=0.5)
            ax1.set_xlabel('COT Predito(%)', fontsize=13)
            ax1.set_ylabel('Residuais (Medido - Predito)', fontsize=13)
            ax1.text(-0.08,1, "B", fontsize=15,fontweight='bold', transform=ax1.transAxes, verticalalignment='top', horizontalalignment='left') 
            #ax1.set_title('Valores Residuais vs Preditos', fontsize=14)
            ...
            p = np.poly1d(z)
            ax1.plot(y_pred_clean, p(y_pred_clean), alpha=0.8, linewidth=1,color="darkred")
            
            # 2. Residuals vs Measured Values
            ax2.scatter(y_true_clean, residuals, alpha=0.8, color='steelblue', s=30)
            ax2.axhline(y=0, color='darkred', linestyle='--', alpha=0.8, linewidth=0.5)
            ax2.set_xlabel('COT Medido(%)', fontsize=12)
            ax2.set_ylabel('Residuais (Medido - Predito)', fontsize=12)
            ax2.text(-0.08,1, "A", fontsize=15,fontweight='bold', transform=ax2.transAxes, verticalalignment='top', horizontalalignment='left') 
            #ax2.set_title('Valores Residuais vs Medidos', fontsize=14)
            ...
            p = np.poly1d(z)
            ax2.plot(y_true_clean, p(y_true_clean), alpha=0.8, linewidth=1,color="darkred")
            
            # 3. Histogram of Residuals
            ax3.hist(residuals, bins=30, alpha=0.7, color='dimgray', edgecolor='black')
            ax3.axvline(x=0, color='darkred', linestyle='--', alpha=0.8, linewidth=1)
            ax3.set_xlabel('Residuais (Medido - Predito)', fontsize=13)
            ax3.set_ylabel('Frequência', fontsize=13)
            ax3.text(-0.08,1, "C", fontsize=15,fontweight='bold', transform=ax3.transAxes, verticalalignment='top', horizontalalignment='left') 
            #ax3.set_title('Distribuição de Residuais', fontsize=14)
            ...
            p = norm.pdf(x, mu, std)
            ax3.plot(x, p * len(residuals) * (xmax - xmin) / 30, linewidth=1.5, alpha=0.8, color="darkred")
            
            # Add metrics text box INSIDE the histogram panel (top left)
            metrics_text = f'RMSLE = {rmsle:.4f}\nMALE = {male:.4f}%\nResidual Médio = {residuals.mean():.4f}\nStd Residual = {residuals.std():.4f}'
            ...
            ax4.lines[0].set_markerfacecolor('silver')
            ax4.lines[0].set_markeredgecolor('dimgray')
            ax4.lines[0].set_markersize(5)
            ax4.lines[1].set_color('firebrick')
            ax4.lines[1].set_linewidth(1)
            for text in ax4.texts:
                 text.remove()
            ax4.text(-0.08,1, "D", fontsize=15,fontweight='bold', transform=ax4.transAxes, verticalalignment='top', horizontalalignment='left') 
            ax4.set_title('', fontsize=14)
            ax4.grid(True, alpha=0.3)
            ax4.set_xlabel('Quantis Teóricos', fontsize=13)
            ax4.set_ylabel('Valores Ordenados', fontsize=13)
            plt.tight_layout()
    ...
    for combo_id, model_names in combinations.items():
        ...
        for i, model_name in enumerate(model_names):
            ...
            if prediction_column in predicted_data.columns:
                ...
                ax = axes[i, 0]
                ax.scatter(y_pred_clean, residuals, alpha=0.6, color='steelblue', s=30)
                ax.axhline(y=0, color='darkred', linestyle='--', alpha=0.8)
                ax.set_xlabel('COT Predito(%)', fontsize=11)
                ax.set_ylabel('Residuais', fontsize=11)
                ax.set_title(f'{model_name} - Residuais vs Preditos', fontsize=12)
                ax.grid(True, alpha=0.3)
                ax.set_xlim(0, 20)
                ...
                ax = axes[i, 1]
                ax.hist(residuals, bins=20, alpha=0.7, color='tan', edgecolor='black')
                ax.axvline(x=0, color='darkred', linestyle='--', alpha=0.8)
                ax.set_xlabel('Residuais', fontsize=11)
                ax.set_ylabel('Frequência', fontsize=11)
                ax.set_title(f'{model_name} - Distribuição Residual', fontsize=12)
                ax.grid(True, alpha=0.3)
                ax.set_xlim(-5, 20)
        ...
        for i, model_name in enumerate(model_names):
            ...
            if prediction_column in predicted_data.columns:
                ...
                if 'COT_anl' in predicted_data.columns:
                    ...
                    if mask_cot_anl.any():
                        ax.scatter(predicted_data.loc[mask_cot_anl, 'COT_anl'], predicted_data.loc[mask_cot_anl, 'DEPTH'],
                                   label="COT Analisado", color='black', marker='*', s=30, alpha=0.7)
                ...
                if 'COT' in predicted_data.columns:
                    ...
                    ax.fill_betweenx(depth, lower, upper, color='darkorange', alpha=0.2, label=f'STD(residuais) window={window}')
...
def check_baseline_predictions(df: pd.DataFrame,
    ...
    print(f"COT Medido: mean={baseline['COT'].mean():.3f}, median={baseline['COT'].median():.3f}, std={baseline['COT'].std():.3f}")
    print(f"COT Predito: mean={baseline[model_col].mean():.3f}, median={baseline[model_col].median():.3f}, std={baseline[model_col].std():.3f}")
    ...
    print(f"COT Predito > 1: {n_pred_acima_1} samples ({perc_pred_acima_1:.1f}%)")
    ...
    if plot:
        ...
        plt.subplot(1, 2, 1)
        plt.hist(baseline["COT"], bins=20, alpha=0.7, label="COT Medido", color="steelblue", edgecolor='dimgray')
        plt.hist(baseline[model_col], bins=20, alpha=0.7, label="COT Predito", color="peru", edgecolor='dimgray')
        plt.text(-0.12, 0.97, 'A', transform=plt.gca().transAxes, fontsize=13, fontweight='bold')
        #plt.title("Distribuição de COT (Baseline)")
        plt.xlabel("COT (%)", fontsize=13)
        plt.ylabel("Frequência", fontsize=13)
        plt.legend()
        plt.subplot(1, 2, 2)
        plt.scatter(baseline["COT"], baseline[model_col], alpha=0.7, color="dimgray", s=7, edgecolor='dimgray')
        plt.plot([min_val, max_val], [min_val, max_val], "r--", label="Ideal", color="darkred", linewidth=0.5)
        plt.text(-0.12, 0.97, 'B', transform=plt.gca().transAxes, fontsize=13, fontweight='bold')
        #plt.title("COT Medido vs Predito (Baseline)")
        plt.xlabel("COT Medido", fontsize=13)
        plt.ylabel("COT Predito", fontsize=13)
...
def check_baseline_predictions_depth(df: pd.DataFrame,
    ...
    if plot:
        ...
        plt.subplot(1, 2, 1)
        plt.hist(baseline["COT"], bins=20, alpha=0.7, label="COT Medido")
        plt.hist(baseline[model_col], bins=20, alpha=0.7, label="COT Predito")
        plt.title(f"Distribuição de COT (Baseline)\nDepth {depth_min}-{depth_max}")
        plt.xlabel("COT (%)")
        ...
        plt.title(f"COT Medido vs Predito (Baseline)\nDepth {depth_min}-{depth_max}")
        plt.xlabel("COT Medido")
        plt.ylabel("COT Predito")
        plt.legend()
        plt.tight_layout()
        if output_dir is not None:
            plot_dir = os.path.join(output_dir, "baseline")
    ...
    for combo_id, model_names in combinations.items():
        ...
        for i, model_name in enumerate(model_names):
            ...
            if prediction_column in predicted_data.columns:
                ...
                ax.barh(depth, y_pred, height=3, color='steelblue', alpha=0.7, label='COT Predito')
                # Pontos pretos para COT medido
                ax.scatter(y_true, depth, color='black', marker='o', s=20, alpha=0.7, label='COT Medido')
                ax.set_xlabel("COT (%)", fontsize=13)
                ax.set_ylabel("Profundidade (m)", fontsize=13)
                ax.set_title(f"{model_name}")
                ax.set_xlim(0, 20)
                ax.invert_yaxis()
                ax.grid(True, alpha=0.3)
